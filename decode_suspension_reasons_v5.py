...
def get_fixture_keys_from_last_n_hours(n: int) -> dict:
...
def check_for_new_suspension_reason_encodings(tables_schemas: list, fixture_keys: list) -> dict:
...
if __name__ == "__main__":
    ...
    try:
        ...
        fixtures, status = get_fixture_keys_from_last_n_hours(48)
        fixtures = format_query_results(fixtures)
        fixtures_list = [fixture[0] for fixture in fixtures]
        print(f'New fixtures - {fixtures_list}') if fixtures_list else ''
        ...
        print(f'New encodings - {encoded_suspension_reasons}') if encoded_suspension_reasons else ''

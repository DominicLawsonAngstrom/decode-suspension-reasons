...
IDENTIFY_TABLES_WITH_SUSPENSION_REASONS_COLUMNS = """
    ...
    WHERE
        ...
        AND (
            column_name = 'selectionsuspensionreason' 
            OR column_name = 'linesuspensionreason'
...
def check_for_new_suspension_reason_encodings(tables_schemas: list[str]) -> dict:
    ...
    for table, schema in tables_schemas:
        CHECK_FOR_NEW_ENCODINGS.append(f'''
            SELECT DISTINCT SelectionSuspensionReason
            FROM
                {schema}.{table}
            WHERE (SelectionSuspensionReason = SelectionSuspensionReason_str)
            UNION    SELECT DISTINCT LineSuspensionReason
            FROM
                {schema}.{table}
            WHERE (LineSuspensionReason = LineSuspensionReason_str)
        ''')
...
def report_job(type: str, name: str, start: str, end: str, exec_time: str, status: str, encodings_added: list):
...
if __name__ == "__main__":
    ...
    new_encodings, status = check_for_new_suspension_reason_encodings(TABLES_SCHEMAS)
